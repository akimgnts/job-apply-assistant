#!/usr/bin/env python3
"""Ingest real jobs from Ashby API - Notion."""
...
print("=" * 80)
print("ASHBY INGESTION — Notion (Real API)")
print("=" * 80)
...
async def ingest():
    ...
    discovered = await adapter.discover_jobs({"company_slugs": ["notion"], "max_per_company": 50})
    print(f"✓ Discovered {len(discovered)} from Notion")
    ...
    print(f"\nAshby Notion samples:")
    ashby_jobs = db.query(JobOffer).filter(JobOffer.source == "ashby").limit(3).all()
    for i, job in enumerate(ashby_jobs, 1):
        location = "N/A"
